...
annotated <- function(data, msg) {
  print(msg)
  data %>%
    nickr_col(!is.nan(score) & score > 0, msg = "scores must be non-negative", logger = print) %>%
    nickr_col(str_detect(person, "[A-Z][A-Z][0-9]"), msg = "subject IDs must be letter-letter-digit", logger = print) %>%
    group_by(person) %>%
    nickr_group(n() == 2, msg = "require two scores per subject", logger = print) %>%
    mutate(adjusted = score - min(score))
