...
FIXME EXERCISE: write an assertion to check that a vector is in sorted order.

## Approximately Correct

The `expect_equal` function can take a third argument
when used with floating-point numbers: `tolerance`.
This tells the function how close values have to be to be considered equal.
It's needed because floating-point numbers are only approximations of real numbers,
so even simple expressions may not produce the mathematically-exact result.
R tries to shield users from this,
...
Since the size of the difference between the true result
and what's calculated using a finite number of bits
will vary depending on the magnitudes of the numbers involved,
`expect_equal`'s `tolerance` value is expressed as a **relative error**,
i.e.,
as a fraction of the actual result.
By default,
the value is roughly 1.5e-8
(see the documentation for `all.equal` for details).
There's usually no reason to change this,
but its existence is the reason why correctness tests should use `expect_equal`
rather than simply `==`.
For more information,
please see [The Floating Point Guide](https://floating-point-gui.de/errors/rounding/).
For *lots* more information,
see David Goldberg's "[What Every Computer Scientist Should Know About Floating-Point Arithmetic](https://doi.org/10.1145/103162.103163)".

FIXME EXERCISE: exercise for floating-point thresholds in testing
...
FIXME EXERCISE: check that other columns in the table aren't accidentally capped.

FIXME EXERCISE: check that no new columns are added to the table
...
FIXME EXERCISE: write a tighter regular expression

FIXME EXERCISE: add checks for null inputs

FIXME EXERCISE: show a simple-minded implementation of `expect_error` and ask what cases it *won't* catch.  This will depend on learners having done the primer on error handling (which in turn depends on me writing it).

## Randomness

There are two rules you absolutely must follow when writing R programs:

1.  [Never use `setwd`](https://www.tidyverse.org/articles/2017/12/workflow-vs-script/).
2.  Always seed the random number generator explicitly so that you can reproduce your results.

The background to the second rule is that computer-generated random numbers aren't really random;
they're produced by a completely deterministic algorithm
whose outputs have all the properties of truly random numbers.
The initial value of such a **pseudo-random number generator** (PRNG) is called the **seed**.
If the PRNG is given the same seed,
it will reproduce exactly the same sequence of values.

This is important because data scientists need to be able to reproduce their work
and to be able to test their software.
Suppose, for example, that you are sampling a uniform distribution 100 times in one of your tests.
If the test fails,
you'll want to be able to re-run it to find out why,
but if you can't regenerate exactly the same set of values,
you might not be able to.

Most programs set the PRNG seed once as the program starts up,
but since `test_that` doesn't guarantee to run tests in the same order every time,
a well-written test suite will re-set the PRNG at the start of each test.
Doing this is usually not necessary,
but few things are as frustrating as having a test that fails once in a hundred runs,
so it's worth the extra line of code.
We will talk more about this when we discussion
[production testing](../production-testing/production-testing.Rmd).
...
FIXME EXERCISE: exercise for organizing tests.
