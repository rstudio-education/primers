...
FIXME EXERCISE: exercise to overwrite the real `getData` with a testing version.
...
3.  It then creates and returns a function that uses that newly-created variable `location` to step through `lines`, just as we did before. The crucial thing is that no global variables are involved.

FIXME: diagram showing function replacement
