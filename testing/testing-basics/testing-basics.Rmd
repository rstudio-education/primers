...
In practice, however, the situation is far from hopeless. Suppose we want to test a function that returns the sign of a number. If it works for 3, it will almost certainly work for 4, and for 5, and so on. In fact, there are only a handful of cases that we actually need to test:
...
2.  **failure**, meaning that the software being tested didn't do what it was supposed to
...
question('Suppose `sign` is supposed to give a warning if it is given a non-numeric value, but instead gives a warning when passed a string like `"0.0"`. How should this be classified for testing purposes?',
...
<img src="images//tdda-errors-5stage-1920x1020.png" style="width:100%" alt="Test-Driven Data Analysis"/>
