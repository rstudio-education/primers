...
`NA` is R's symbol for missing information, and it is common to have multiple `NA`s when you reshape your data from a wide format to a long format. The two by two table structure imposed by wide data requires a place holder for every combination of variable values---even if no data was collected for that combination.
