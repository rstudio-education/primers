...
* `select()` - which reshapes long data into wide data
...
So far we've used numbers to describe which columns to reshape with `gather()`, but this isn't necessary. `gather()` also recognizes column names as well as all of the `select()` helpers that you learned about in [Isolating Data with dplyr](). So for eaxmple, these expressions would all do the same thing:
...
```{r ex8-check}
"Good job! Together the tidy versions of table4a and table4b repeat the information in table1. In Join Data Sets you will leanr how to combine them back into a single data set."
```
...
```{r ex13-check}
"Good job! You can see that the data shows that more boys than girls were born for the years prior to 1936, and more girls than boys for the years after 1936."
```
...
To do this, we needed a data set that placed $$year$$, $$total$$, and $$color$$ each in their own columns.
