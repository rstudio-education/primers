...
* Try running `log()` on the lists below, and find out.
...
How likely is it that an R function will work with both `1` and the vector `c("a", "b", "c")`? Most R functions only work with a single type of data, so their authors arrange for them to handle simple vectors, but not lists.

### What should you do?

Don't worry, there's no reason to go back to repeating things by hand for lists, although you could. Couldn't you? Let's see. 

You can extract each element of a list with the `[[i]]` bracket syntax. For example, this extracts the second element of `list2`.
...
* Change the code below to map `log()` over the elements of `list1`.
...
```{r map1-check}
"Excellent. You can use map() with any function to iterate over the elements of a list. Your map() code returned the same results as the manual code you wrote earlier, except these results are returned in a list."
```
...
For example, if you wanted to round to a certain number of digits past the decimal place, you would give `round()` an extra argument named `digits`, e.g. `round(3.141593, digits = 2)`.

### \t

You can pass extra arguments _to_ `map()` as if they were arguments _of_ `map()`. `map()` will forward the arguments to your function whenever it calls the function. You will almost always want to call these arguments by name, e.g.
...
What if you wanted to return the results as a regular vector (i.e. a non-recursive vector, a vector that is not a list)?
...
```{r map_dbl-check}
"Good job. Now our results come back as a numeric vector. This is useful. For example, we could pass the results directly to round() without using another call to map()."
```
...
```{r ldeaths-1-check}
"This is the data we will explore. ldeaths is a reformatted version of the ldeaths time series data that comes in base R."
```

### Deaths by year

It would be interesting to know how many people died each year from lung disease. Can you find out?

* Use a map function to compute the sum of each vector in `ldeaths`. Then click Submit Answer. Did the deaths increase or decrease over time?
...
But each expects to iterate over a set of _data frames_ and `ldeaths` contains a set of vectors. To make `map_dfr()` work, we will need to use it with a function that returns data frames for `map_dfr()` to combine.

### as_tibble()

R has many functions that turn a vector into a data frame: `data.frame()`, `as.data.frame()`, `data_frame()`, `as_data_frame()`, etc. 
...
* Use `map()` to map `as_tibble()` over the vectors in `ldeath`. Then click Submit Answer. (For now, use `map()` instead of `map_dfr()`, this will help us look at what happens step-by-step as we wrangle this data.)
...
`as_tibble()` turned each vector into a data frame with one _column_. If we want the data frame below, we'll need each vector to become a data frame with one _row_ (that way `map_dfr()` can stack the rows on top of one another).
...
If we try running `map_dfr()` on our current output, it will stack the columns on top of one another to make a data frame with one long column. Go ahead, try it.
...
Don't be fooled by the way R displays the vectors. R only thinks of the second vector as a row vector. You can tell by the bracket notation in front of the vector. It indicates that this is the first _row_ of the result (but you can ignore that if it doesn't interest you).
...
_(You may need to first complete the Work with Data Primer to understand these final three sections.)_
...
* Use `summarise_all()` and the code below to sum across months. Then click Submit Answer.
...
Congratualtions on making it to the end. You now know the ins and outs of iterating with `map()`, but there is more to learn.
