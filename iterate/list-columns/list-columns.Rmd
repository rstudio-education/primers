...
```{r data-list, exercise = TRUE, exercise.eval = TRUE}
nested_gapminder %>%  
  pluck("data")
```
...
```{r models-lc-check}
"You did great! As you can see, mutate() and the other tidyverse functions know how to work with list columns. If the input to mutate() creates a list, mutate() saves the result as a list column."
```
...
```{r dbl-hint-1}
"You learned about map functions that return different types of output in the Introduction to Map tutorial."
```
