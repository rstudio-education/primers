...
The map functions come with several very useful shortcuts as well as new functions that let you map over two or more lists, or even a list of functions. 
...
Even though it is not related to `map()`, it would be rewarding to learn which countries had the largest and smallest changes in life expectancy. Let's take a moment to find out.
...
The alternative is to write an expression in your head, and to then see if it works. Too often it won't. 
...
Syntactically, `map2()` behaves like `map()`, but it takes _two_ lists as arguments before it takes a function. At each step of the iteration, `map2()` will pass an element from the first list to the first argument of the function. It will pass an element from the second list to the second argument of the function.
...
Name each list in your list of lists with the name of argument it should go to. `pmap()` will match names to arguments whenever you provide them, e.g.
...
(if you do not provide list names, `pmap()` will map lists to arguments by order).
