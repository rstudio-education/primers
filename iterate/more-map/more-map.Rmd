...
The map functions come with several incredibly useful shortcuts as well as new functions that let you map over two or more lists, or even a list of functions. 
...
* Click Submit Answer to examine the contents of `us`. Can you find the life expectancy for a US citizen in 1952? How about in 2007?
...
```{r us-1-check}
"Good job. Life expectancy statistics are saved in the second element of us: the element named lifeExp. The first number in lifeExp is the life expectancy in 1952, the last number is the life expectancy in 2007. Ideally, you'd encounter this type of data in a data frame, but it is not uncommon to receive messy data like this."
```

### \t

Let's use the values of `us$lifeExp` to compute the change in life expectancy from 1952 to 2007. 

This is a simple task, but we can use it as the basis of an iteration project.

### pluck()

At the moment, the values of `lifeExp` are hard to use because they are stored inside of a list. Purrr's `pluck()` function can help us extract the values so that we can manipulate them directly. 

### \t

`pluck()` extracts an element from a list by name or by position, e.g. `pluck(list1, var = 1)`, `pluck(list1, var = "numbers")`.
...
Now that you have the values of `us$lifeExp` we can do our calculation. Here are some functions that will help:

1. `first()` extracts the first element of a vector. 
2. `last()` extracts the last element. 

Both come in the dplyr package, which I've pre-loaded for you.

### \t

$$\text{change in life expectancy} = \text{life expectancy in 2007} - \text{life expectancy in 1952}$$

* Use `first()` and `last()` to compute the change in `lifeExp` from 1952 to 2007. Then click Submit Answer.

```{r us-2, exercise = TRUE}
lifeExp <- us %>% pluck(var = "lifeExp")
...
Guess what? You can. `gap_list` contains statistics for 142 countries, but to iterate through them, you will need to use map functions---and their shortcuts. Click Run Code to see.
...
Both of these steps will reveal new aspects of the map functions. 
...
gap_list %>% map(pluck, var = "lifeExp")
```
...
Will our shortcuts work for `gap_dfs`?

* Try it. Use `map()` and the "" shortcut to extract the `lifeExp` column of each sub-data frame. Then click Submit Answer. What happens?
...
```{r gap_dfs-1-check}
'The "" shortcut works with data frames. Now lets try the integer shortcut.'
```

### \t

* Rewrite your code to use the integer shortcut. Can you still extract the `lifeExp` columns?

```{r gap_dfs-2, exercise = TRUE}
gap_dfs %>% map("lifeExp")
```

```{r gap_dfs-2-solution}
gap_dfs %>% map(2)
```

```{r gap_dfs-2-check}
"Both shortcuts work with data frames. And the reason is simple: in R data frames are a type of list, just like tibbles are a type of data frame. As a result, things that work with lists will also work with data frames."
```

### One more thing

There is one more thing that you should know about these shortcuts. In the Map tutorial, you learned that `map()` is just one part of a larger family of map functions:
...
Step 1 was easy and you learned two useful shortcuts along the way, but Step 2 will be harder.

### Step 2

Your current results are a list of lifeExp vectors.
...
Second, wherever the element to manipulate appears in the expression replace it with `.x`. This tells `map()` where to use the elements within your expression. If an expression uses each element multiple times, you will need to insert multiple `.x`s into your expression.
...
map(lists, ~rnorm(5, mean = pluck(.x, var = 1), sd = pluck(.x, var = 2)))
```
...
```{r rnorm-check}
"Nice. Don't be confused by the element names, all of these numbers come from a normal distribution."
```

### Use it

Now its your turn. To finish your code: 

1. Turn `last(lifeExp) - first(lifeExp)` into an expression.
2. Use `map()` to apply the expression to each element of our current result. 
...
```{r exp-check}
"Great job. This expands you power to use map() greatly; you can do almost anything with the right expression."
```
...
Even though it is not related to `map()`, it would be very rewarding to learn which ountries had the largest and smallest changes in life expectancy. Let's take a moment to find out.
...
2. `top_n()` arranges a data set by a column and then returns the top n rows. 

    ```{r}
    top_n(mtcars, n = 5, wt = mpg)
    ```
    
    Notice that `top_n()` returns the top n rows, but it displays them in the order that they appear in the data set (don't be fooled: these are the rows with the five wighest values of `mpg`. `top_n()` retrieves them but does not sort them by `mpg`).
...
3. You can combine `top_n()` with `desc()` to retrieve the lowest n values.
    ...
    Both `top_n()` and `desc()` come in the dplyr package, which I've pre-loaded for you.
...
Another way to quantify the change in life expectancy over time is to fit a simple model to the data. Let's fit a very simple model to each data set: the linear regression of `lifeExp` on `year`. 

The slope of this model will be how fast life expectancy increased per year, on average.
...
I'm not sure if you've ever fit a linear model with r before, so I'll just tell you: this is the code that we want. 
...
* Transform your code into an expression and then use `map()` to apply it to each element of `gap_dfs`. Then click Submit Answer.
...
Let's revist our models and account for the effect of economic growth on life expectancy.
...
How can you tell? The very last number in the table is a p-value (here 0.1424). If the number is above 0.05, there's not enough evidence to suggest that the second model outperforms the first: you can ascribe any difference between the two to random chance.
...
To find out you can iterate through all of the countries, but this is a new sort of iteration problem. At each step, `anova()` will require one element from `model1` and one element from `model2`. You'll need to simultaneously iterate over _two_ lists.

### map2()

Enter `map2()`.

`map2()` takes _two_ lists and then a function. At each step of the iteration, `map2()` will pass an element from the first list to the first argument of the function. It will pass an element from the second list to the second argument of the function.
...
Syntactically, `map2()` behaves like `map()`, but it takes _two_ lists as arguments before it takes a function.
...
For another example, `map2()` returns its results as a list, but it comes with variants that will return the result in other formats. As with `map()`, you should use the variant of `map2()` that will return your results in the format you want:
...
2. Refer to elements from the first list as `.x`
3. Refer to elements from the second list as `.y`
...
### Mapping over three lists

Anova can handle three arguments. Can you?

Yes, but not with `map3()` as you might think. `map3()` doesn't exist. Instead purrr offers the `pmap()` function for mapping over three _or more_ lists.

### pmap()

The syntax of `pmap()` is a little different from the syntax of `map()` and `map2()`. Instead of accepting lists one at a time as arguments, `pmap()` expects a single argument that contains a _list of lists_ and then a function to apply to the lists within that list of lists.
...
If you do not name the lists within your list of lists, `pmap()` will pass elements of the first list to the first argument of your function. It will pass elements from the second list to the second argument of the function. And so on, e.g.

```{r}
nums <- list(pi, exp(1), sqrt(2))
digits <- list(2, 3, 4)
pmap(list(nums, digits), round)
```

### \t

However, if you have many lists to iterate over, it can be hard to keep track of argument positions. In this case, a better approach would be to name each list in your list of lists with the argument it should go to. `pmap()` will match names to arguments whenever you provide them, e.g.
...
_Note: `anova()` does not use argument names, so you will need to supply your lists in the correct order._
...
```{r pmap-check}
"Excellent. You can use pmap() to map over three, four, or any number of lists."
```
...
For example, you could return the year coefficients of each model with:
