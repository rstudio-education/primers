...
To see how useful this technique can be, let's use multi-vector mapping to compare two lists of models. At the end of the Map Shortcuts tutorial, you fit two models for every country in `gap_dfs`:

1. A model that predicted life expectancy by year, which I saved as `model1`.
1. A model that predicted life expectancy by both year _and GDP per capita_, which I saved as `model2`.
...
```{r map2-check}
strict_check("Nice job! A quick scan of the results suggests that adding gdpPercap only improved your predictions for the poorest countries.")
```
...
```{r map2-exp-check}
strict_check("Great work! Click Continue to learn how to make these results even easier to work with.")
```
...
```{r enframe-check}
strict_check("Bravo! There was a lot to the last two exercises and you handled them purrrfectly.")
```
...
```{r pmap-check}
strict_check("Excellent. You can use pmap() to map over three, four, or any number of vectors.")
```
...
Because a data frame is a list, you can pass a data frame to the first argument of `pmap()`. `pmap()` will do something natural when you do this...

### rowwise operations

`pmap()` applies a function to each row of a data frame.
...
```{r pmap-gen-check}
strict_check("Good job! Remember that pmap() is a useful tool for doing rowwise operations on data frames (and tibbles, since tibbles are data frames too).")
```
...
Now, let's push it one step further. What if you wanted to use a different simulation function for wach row? For example, you could use:
...
```{r invoke-check}
strict_check("Very interesting! But what if you would like to supply additional arguments to your functions?")
```
### Arguments
...
```{r unnamed-check}
strict_check("Good job thinking outside of the box! Remember though that it is always best practice to include argument names (whenever it does not result in an error).")
```
