...
1. purrr functions also let you do things that you cannot do with the `lapply()` family. For example, you can map over two lists with `map2()`, or the rows of a data frame with `pmap()`. You can also arrange to return input invisibly as you trigger side effects with `walk()`.
