...
Give `labs()` a subtitle argument to add a subtitle. If you use multiple areguments, remember to separate them with a comma.
...
To change the theme of your plot, add a `theme_` function to your plot call. The ggplot2 package procides eight theme functions to choose from.
...
Everytime you map an aesthetic to a variable, ggplot2 relies on a _scale_ to select the specific colors, sizes, or shapes to use for the values of your variable. 
...
ggplot2 supplies over 50 scales to use. This may seem overwhleming, but the scales are organized according to an intuitive naming convention.
...
By default, `scale_color_brewer()` will use the "Blues" palette from the RColorBrewer pacakge. To use a different RColorBrewer palette, set the palette argument of `scale_color_brewer()` to one of the RColorBrewer palette names, surrounded by quotation marks, e.g.
