...
So for example, the code below performs an implied grouping. And since we use the color aesthetic, the plot includes a legend, the color legend.
...
1. Dfollow `geom_map()` with `expand_limits()`, and tell `expand_limits()` what the $x$ and $y$ variables in the map dataset are. This shouldn't be necessary in future iterations of `geom_map()`, but for now ggplot2 will use the x and y arguments of `expand_limits()` to build the bounding box for your plot.
