...
You should not supply a $y$ aesthetic when you use `geom_bar()`; ggplot2 will count how many times each $x$ value appears in the data, and then display the counts on the $y$ axis. So, for example, the plot above shows that ove 20,000 diamonds in the data set had a value of `Ideal`.
...
To create the familiar stacked bar chart, set `position = "stack"` or rely on the default position value. For `geom_bar()` this is `position = "stack"`.
...
Position adjustments tell ggplot2 how to re-distribute objects when they overlap. `position = "identity"` is the "adjustment" that let's objects overlap each other. `position = "identity"` is a bad choice for bar graphs because the result looks like a stacked bar chart, even though it is not. 
