...
This very short tutorial will show you how R functions are packaged and how you can work with R functions. It is a pre-requisite for the next tutorial, which will show you how to write your own R functions. Let's begin!
...
```{r xor-body-check}
strict_check("xor contains one line of code in its body (you can ignore the braces). This isn't always the case. A function can have as many lines of code as it needs in its body. If you don't believe me check out the body for lm. R will run the code in a function's code's body each time you call the function; but as you can see, the code in a function's body may not be self-contained. What is x? What is y? Click Continue to find out.")
```

### formals()

Often times, a function will need one or more values to do its job. For example, `xor` cannot return a result unless you provide it both a value for `x` and a value for `y`. These objects are the _formal arguments_ of the `xor` function
...
```{r xor-formals-check}
strict_check("A function can have zero, one, or many many formal arguments. Do not be distracted by the $'s that appear before the argument names. Those are an artifact of how R displays lists.")
```
...
```{r gather-env-check}
strict_check("The gather() function comes in the tidyr package. If a function's environment is namespace:base, that is a good sign that the function comes in base R, the part of R that loads before you load any optional packages.")
```
...
```{r rnorm-check}
strict_check("rnorm provides a default value of zero for the mean argument and a default argument of one for the sd argument. In other words, if you do not supply a mean and sd value when you call rnorm, rnorm will randomly generate results from a standard normal distribution. Default values can be very handy!")
```
