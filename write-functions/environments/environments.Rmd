...
`globalenv()` is a function that returns an R environment. In fact, `globalenv()` returns a very specific R environment named the _global environment_. You'll learn more about the global environment later, but first take a look at how R displays environments. 
...
```{r ls.str-check}
strict_check("I've saved three objects in the global environment named `x`, `y`, and `z`. They have the values `1`, `2`, and `3`. ls.str() provides a good depiction of an environment, because ls.str() shows the contents of the environment. However, ls.str()'s depiction of the global environment is missing one thing.")
```
...
```{r parent-check}
strict_check("You can use `parent.env()` to find the parent of any environment in R, but notice that there is no way to find the children of an environment: each child knows who its parent is, but each parent does not know who its children are. Compare this to real life: you can know who you are pointing at, but you can't know who may be pointing at you. (You can ignore the extra attributes that appear with this environment label).")
```

### parenvs()

The parent of an environment will also have a parent, and so on. Together the parents will form a chain of environments that ends with the _empty environment_, which is the only environment in R that does not have a parent.

Every environment can trace its lineage to the empty environment through a set of parents.

You can see an environment's lineage with the `parenvs()` function that comes in the `pryr` package. Give `parenvs()` the argument `all = TRUE`; this tells `parenvs()` to display the parent of each parent. `parenvs(e, all = TRUE)` will display the parent of each environment beginning with whichever environment you pass to `e`.

* Use `parenvs()` to see the "parentage" of the global environment. Then click Submit Answer.
...
```{r search-check}
strict_check("The global environment is at the top of this list. The second line contains the parent of the global environment. Next comes the parent of that parent, and so on until we reach the empty environment.")
```
...
1. If you create a new object, R will store it in the active environment. 
2. If you call an object, R will look for its value in the active environment. 

What is R cannot find the value in the active environment?
...
As a result, the global environment acts as your personal workspace: it is where R will save objects that you create at the command line, and it is where R will look for objects that you call at the command line.
...
You can see each of these in the search path from the active environment of the exercise chunk below.

* Use the exercise chunk to display the search path from the active environment (i.e. the list of parents that connects the active environment ot the empty environment). Then click Submit Answer.
...
The search path will also vary based on which packages you have loaded. The environments of loaded packages appear between the active environment and the empty environment in the reverse order of which the packages were loaded. 
...
```{r x-check}
strict_check("This is my password. Shhh! Don't tell! It is very important.")
```
...
What would happen if you run the code below?
...
```{r x2-check}
strict_answer("Remember that R saves the objects that you create to the active environment. If you use a name that already exists in the active environment, R will overwrite the object with that name. Here R overwrote x = password123 with x = oops. This is a true oops because you will not be able to get x = password123 back: it is gone.")
```
...
```{r x3-check}
strict_check("x = password123 is stored in the global environment, which will be the parent environment for each of the exercise chunks that follow.")
```
...
```{r x4-check}
strict_answer("But uh oh: the same result. This time things are not as dire. R saved your version of x to the active environment, which means that it did not overwrite the version of x in the global environment. However, when R searches for x R will find your version of x and return it, without searching further up the search path.")
```

### \t

This behavior is called masking. _Masking_ occurs whenever two objects with the same name exist in the search path. When masking happens, R will always use the object that apears first in the search path. 

Masking can be confusing if you do not realize that it is happening.

***

Tip: you can remove the version of `x` in the active environment with `rm(x)`.
...
1. **Overwriting** happens when you assign a new value to a name that already exists in the active environment

2. **Masking** happens when you create an object that has the same name as an object further down the search path
...
question("TRUE or FALSE: Every function is called from some calling environment.",
         answer("TRUE", correct = TRUE, message = "R is always working closely with an active environment. Whichecver environment is active when you call a function will be the function's calling environment."),
         answer("FALSE"),
         allow_retry = TRUE)
```
...
question("What would happen if R ran the code saved in `foo()` in `foo()`'s calling environment?",
         ...
         answer("If an object named z existed in the calling environment, foo() would overwrite its value with 3.", correct = TRUE, message = "foo() will pollute the calling environment with an object named z. This could overwrite an object named z in the calling environment, or mask an object named z in another environment. R avoids this problem by running foo() in a fresh execution environment."),
         answer("An error would occur."),
         answer("foo() will check to see if an object named z already exists in the calling environment. If so, it will use that value of z."),
         allow_retry = TRUE)
```

### Could the execution environment stay active?

Suppose there _is_ an object named `z` stored in the calling environment. In fact, `z` contain my password, `"password123"`. Boy am I glad that R runs the body of `foo()` in a fresh execution environment where, `z <- 3` cannot overwrite my password!
...
question("What would happen if one function called another function in its code body?",
         ...
         answer("R would make the calling environment active and return an error."),
         answer("R would ignore the call to the second function."),
         answer("That thing from Ghostbusters would occur. The thing where they cross the beams.")
```
...
The `show_stack()` function comes in the envtutorial package, which is a package I made specifically for this tutorial. `show_stack()` shows the call stack at the moment it is called. The call stack does not look very impressive when you call `show_stack()` directly from the active environment (which in my case is the global environment):
...
Eventually, R had finished running all of the code and had removed every environment but one from the call stack. At that point, the active environment was the global environment.
...
question("Will the call stack that leads to a function be the same as the search path used by the function?",
         answer("yes"),
         answer("No", correct = TRUE, message = "The call stack that leads to a function can vary from one run of the function to the next (as you saw with show_stack()), but you want the function to always use the same search path each time it runs. Let's see why."),
         allow_retry = TRUE)
```
...
If R uses the call stack as its search path, R will find and use the incorrect version of `show_stack()`. 
...
```{r show-check}
strict_check("The enclosing environment of show_stack() is the envtutorial package, but the enclosing environment for any function that you create at an R console command line will be the global environment.")
```
...
Everytime R runs a function, R will create a new execution environment; but each of these execution environments will use the same parent environment: the enclosing environment of the function.

As a result, the function will always run with the same search path, finding the same values for undefined variables each time.

In my example, `i()` was defined in the global environment, which became its enclosing environment. When R needed to look up `show_stack()` it first looked in the execution environment of `i()`, and then in the global environment(), bypassing the execution environment of `j()`.
