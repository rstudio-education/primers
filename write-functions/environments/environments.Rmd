...
Suppose there is already an object named `z` stored in the active environment. This `z` contains the important string `"password123"`. What would happen to `z` when you call `foo()`?

```{r fq1, echo = FALSE}
question("If R runs `foo()` in the active environment, which of the following would happen?",
         answer('R will overwrite z = "password123" with z = 3.', correct = TRUE, message = "z poses a problem: R must execute the code in foo(), but if R executes the code in the active environment, foo() will add an object named z to the environment. This would overwrite the object named z that already exists in the environment."),
         answer('R will not run z <- 3, because R notices that an object named z already exists.'),
         answer("R will create a new object named z_2 that has the value of 3."),
         answer("An error."),
         allow_retry = TRUE)
```
...
R avoids this chaos by creating a new environment to run `foo()` in. In fact, each time you call a function in R:
...
By running functions in fresh environments, R ensures that the functions will not overwrite previously existing objects.

I'll call the environment that is active when you call a function the _calling environment_.

I'll call the new environment that R makes to run the function in an _execution environment_, because R uses this environment to execute the function. 

### \t

To summarize, when you run `foo()`, R creates an execution environment to run the body of `foo()` in, i.e.
...
When `foo()` has finished running, this execution environment contains a single object, `z = 3` and the calling environment retains its object, `z = password123`.

### Do execution environments stay active?

Suppose that R does nothing else, and the execution environment remains the active environment.

```{r fq2, echo = FALSE}
question("What would happen if you now run `z` to retrieve your password?",
         answer("R will return 3, because the new value of z masks the old value.", correct = TRUE, message = "The new value of z exists in the execution environment, which is the active environment, which is the first environment on the search path. R finds the new value of z and stops searching. This would be a problem. As you can guess, this is not how R works. But how does R work?"),
         answer("R will return password123."),
         answer("R will chastise you for using such a silly password."),
         answer("R will return an error message"),
         allow_retry = TRUE)
```

### Execution environments do not remain active

An execution environment prevents `z <- 3` from overwriting `z = "password123"` in the calling environment, but `z <- 3` would still mask `z = "password123"` if the execution environment remains active.

R solves this problem with a simple action: whenever R finishes running a function, R switches the active environment back to the calling environment. R switches the active environment back to the calling environment each time R finishes running a function. If the function returns a result, this result will be passed back to the calling environment (where you can save it with `<-`).

### Execution environments and the search path

Because execution environments do not remain active, and because execution environments are not the parent of the calling environment (or any of its ancestors), execution environments do not remain on the search path once a function has finished running. 

Execution environments are literally safe, out-of-the-way environments to run functions in. Objects that a function creates will not mask or overwrite existing objects, nor will they appear in the search path. This last fact is a double edged sword: it means that the objects created by a function cannot be easily retrieved, so be sure to have your function return anything of importance in its result.

### show_env()

Let's take a peek at an execution environment in action. `show_env()` is a function in the `envtutorial` package, which is a package that I made specifically for this tutorial.
...
The second line of `show_env()` will capture the environment that is active while the code in `show_env()` runs. This will be the execution environment for that run of `show_env()`.
...
* Click Submit Answer to see if you are right.
...
```{r show2-check}
strict_check("environment() will always return the name of the calling environment because R always makes the calling environment active again after it runs show_env(). Think of the execution environment as a temporary side trip or errand: R begins in the calling environment, switches to the execution environment, and then returns back to the calling environment.")
```
...
Call stacks create another dilemma for R. Consider the function `i()`. It calls a function named `show_stack()`. `show_stack()` is not defined in the execution environment of `i()`, but `show_stack()` is on the search path:

```{r}
parenvs(all = TRUE)
```

* Use a function from the pryr package to determine where `"show_stack"` lives on the search path. Then click Submit Answer.

```{r whereshow, exercise = TRUE}

```

```{r whereshow-hint}
"Surround show_stack with quotation marks")
```

```{r whereshow-solution}
where("show_stack")
```

```{r whereshow-check}
strict_check("show_stack() lives in the package environment for the envtutorial package.")
```

### \t

The search path above is what the search path looks like from the global environment. What should the search path look like from the execution environment of `i()`?

On first guess, you might assume that R would use the call stack as the search path (well the call stack to search to the global environment, and then the search path from the global environment to search to the empty environment). But this sensible approach would not work.

To see why, let's write some new code.
...
When R runs `i()` (after running `k()` and `j()`), R will begin to search for the function named `show_stack()`. If the search path followed the call stack, R would find the function `show_stack <- function() 1 + 1`, which lives in the execution environment of `j()` before R would find the `show_stack()` that lives in the envtutorial package environment.

Since any function can use any name, there is no way to ensure that a function further up the call stack does not create objects that mask objects needed by a function further down the call stack.
